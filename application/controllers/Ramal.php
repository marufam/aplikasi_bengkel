...
class Ramal extends CI_Controller
{
    ...
    function index(){
        ...
        if ($cari <> '') {
            $config['base_url'] = base_url() . 'ramal/index.html?cari=' . urlencode($cari);
            $config['first_url'] = base_url() . 'ramal/index.html?cari=' . urlencode($cari);
        } else {
            $config['base_url'] = base_url() . 'ramal/index.html';
            $config['first_url'] = base_url() . 'ramal/index.html';
        }
        ...
        $this->load->view('ramal/ramal', $data);
         $this->load->view('foot');
    }
    function index2(){
        $this->load->view('nav');
        ...
        $this->load->view('ramal/ramal_uji', $data);
         $this->load->view('foot');
    }
    function peramalan($id){
        ...
        if(isset($_POST['lihat'])){
        	...
        	$alpha = $_POST['alpha'];
        	// $data=$this->Penjualan_model->jumlahdata_plus($id, $datatahun);
	    	// for ($i=1; $i <= 12; $i++) { 
            ...
            // var_dump($data2);
            $this->load->view('ramal/peramalan', $data);
        }else{
        	if ($row) {
            $data = array(
				'kode_barang' => $row->kode_barang,
    ...
    function peramalan_uji($id){
        ...
        if(isset($_POST['lihat'])){
            ...
            // var_dump($data2);
            $this->load->view('ramal/peramalan_uji', $data);
        }else{
            if ($row) {
                $data = array(
                    'kode_barang' => $row->kode_barang,
    ...
    function ramal($id, $tahun, $bulan, $alpha){
        ...
        $data=$this->Penjualan_model->jumlahdata_plus($id, $tahun, $bulan);
        $data_min=$this->Penjualan_model->jumlahdata_min($id, $tahun+1, $bulan);
    	
        $data_depan=$this->Penjualan_model->jumlahdata_plus($id, $tahun+1, $bulan);
        $data_min_depan=$this->Penjualan_model->jumlahdata_min($id, $tahun+2, $bulan);
        ...
        foreach ($data_depan as $data1) {     
            $data_depan[$i]= array("bulan"=>$data1->bulan, "tahun"=>$data1->tahun, "jumlah"=>$data1->jumlah );
            $i++;
        }


        // var_dump(12-sizeof($data));
        $m=12-sizeof($data);
        for ($i=0; $i < $m; $i++) { 
            array_push($data, $data_min[$i]); //menggangubngkan dua antara datamin dan data
        }

        $i=0;
    	foreach ($data as $data1) {		
    		$data[$i]= array("bulan"=>$data1->bulan, "tahun"=>$data1->tahun, "jumlah"=>$data1->jumlah );
            $i++;
    	}
    	...
    	if(isset($data_depan)){
	    	...
	    	$MAE_avg=0;
	    	for ($i=0; $i < 12; $i++) { 
	    		$MAE[$i] = abs($data_depan[$i]['jumlah']-$awal5[$i]);
	    		$MAE_sum+=$MAE[$i];
	    	}
	    ...
	    //=======================
	    	...
	    	$MSE_avg=0;
	    	for ($i=0; $i < 12; $i++) { 
	    		$MSE[$i] = pow(($data_depan[$i]['jumlah']-$awal5[$i]),2);
	    		$MSE_sum+=$MSE[$i];
	    	}
    	...
    	if(isset($awal5)){
	    	...
	    	$MAPE_avg=0;
	    	for ($i=0; $i < 12; $i++) { 
	    		$MAPE[$i] = abs(($awal5[$i]-$data_depan[$i]['jumlah'])/$data_depan[$i]['jumlah']);
	    		$MAPE_sum+=$MAPE[$i];
	    	}
	    	...
	    	//=======================
	    	for ($i=0; $i < 12; $i++) { 
	    		$data[$i]['ril'] = $data[$i]['jumlah'];
                $data[$i]['tahun_yang_diramal'] = $data_depan[$i]['jumlah'];
